...
- 🏆 **国赛备赛知识库**：内建 2026 国赛规则与 AI 合规、题型识别与模型选型口诀、论文写作与三类检验规范、降 AI 痕迹写作要点，覆盖参赛规则到提交自查的完整知识链。
- 🧭 **进度感知与主动建议**：每次交互先判定当前进度（读题→建模→编程→论文→自检→答辩），主动给出下一步建议；拿到题目先给读题想法，跑出结果先解读结论，交付论文后主动复查并引导自查与查重/AI 率检测。
...
`1.4.0` 新增进度感知与主动建议协议（读题先行反馈、结果即时解读、论文交付后主动复查与自查/查重引导），详细内容见 [CHANGELOG.md](CHANGELOG.md)。
