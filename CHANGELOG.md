...
- 新增 `references/实战复盘与用户偏好.md`：沉淀 E 题实战中的方法复杂度、公式解释、图表全量展示、标签遮挡、图例可读、雷达图取舍、题目分析 HTML、2026 AI 合规和 Word/PDF 导出避坑。
- 根 `SKILL.md`、`references/README.md`、建模手、编程手、论文手和可视化工具新增按需读取入口；以后用户要求沿用个人偏好或避免上次返工问题时优先读取。
- 强化用户偏好：优先常见可答辩方法，在数据处理、约束设计、权重扫描和检验方式上做轻量创新；图表默认覆盖全部对象，公式必须配白话解释。
- 新增小问关联与全文逻辑门禁：建模前整理依赖链，逐问说明对后续问题的输入或约束贡献，禁止把题目未给出的背景、变量、约束、评价口径或数据来源写成事实；论文写作必须保持术语、符号、单位和结论口径一致，保证评委通读时顺读。

### 2026 CUMCM 格式与 AI 合规同步

- 校正正文页数口径：正文从纸质版第 4 页开始计算，摘要页和附录不计入正文页数；不得用 PDF 总页数或"摘要到参考文献"页数替代正文页数。
- 同步 AI 工具使用声明要求：无论是否使用 AI，均须在参考文献前设置声明；使用 AI 时支撑材料包含《AI 工具使用详情.pdf》。
- 明确 2026 试行规则下，AI 工具生成或辅助生成内容不需要在正文逐处标注，也不需要把 AI 工具列入参考文献。

## 1.4.0 - 2026-08-27

### 进度感知与主动建议

- 新增 `references/进度状态机与主动建议.md`：定义 S0–S6 六阶段进度状态、判定方法（对话上下文 + PROJECT_ROOT 产物）、每轮交互的固定输出格式（当前状态/刚完成/下一步建议/用户待办），以及关键节点主动行动。
- 根 `SKILL.md` 新增"进度感知与主动建议"协议并接入路由与渐进式加载；description 同步更新。
- 建模手新增"读题先行反馈"：拿到题目与附件后第一轮回复必须先给题型判断、每问目标、数据观察、初步思路、风险点与待确认项。
- 编程手新增"结果即时解读"：得到真实结果后先输出关键数值、合理性检查与下一步建议。
- 论文手新增"交付后主动复查"：论文交付或用户反馈后主动打开论文文件复查，输出缺陷清单、自查引导与查重/AI 率检测正确做法（官方无统一合格线，检测结果只作风险信号）。
- `references/README.md` 与 README 能力清单同步更新。

## 1.3.0 - 2026-08-27

### 国赛备赛知识库

- 新增 `references/2026国赛规则与AI合规.md`：2026 届时间与报名、论文格式硬约束（承诺书/编号页/摘要页/正文 30 页/20MB 上限）、AI 工具使用声明与《AI 工具使用详情.pdf》要求、提交前核对清单。
- 新增 `references/题型识别与模型选型.md`：六大题型判定表（评价/判别/分组/预测/最优/机理）、易混词辨析、A/B/C 选题规律与官方评审标准、模型选型口诀、高分组合与选型纪律。
- 新增 `references/论文写作与检验规范.md`：标准结构、摘要写法、图表公式规范、模型检验三件套（误差/灵敏度/稳健性）、降重与降 AI 痕迹写作、交付前自查。
- 根 `SKILL.md` 路由表和渐进式加载表新增对应入口；`references/README.md` 导航同步更新；README 能力清单补充国赛备赛知识库条目。
## 1.2.0 - 2026-08-10
