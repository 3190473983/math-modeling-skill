---
name: math-modeling
description: 当用户要求数学建模、建模竞赛、建模分析、代码求解、结果可视化或生成数学建模论文时使用。支持完整三阶段流程、单独执行任一角色，以及在关键节点使用独立 Subagent 进行阶段内质检；默认只启用质检 Subagent，用户可明确选择额外协作。内建国赛规则与 AI 合规、题型选型、论文写作与三类检验知识库，并具备进度感知与主动建议能力：拿到题目先给读题想法，交付论文后主动复查并引导自查与查重。
---
...
用户要求沿用个人习惯、复盘既往问题或“别再犯上次问题”时，读取 `references/实战复盘与用户偏好.md`。该文件只沉淀可复用偏好和避坑清单，不复用旧题内容；官方规则和用户最新指令始终优先。
...
本 Skill 在每次交互中都应能回答"现在进行到哪、下一步该做什么"：

1. 收到用户新输入后，先按 `references/进度状态机与主动建议.md` 判定当前状态（S0–S6），再输出：当前状态、刚完成/刚发现、下一步建议、需要用户做的事。
2. 拿到题目与附件后，第一轮回复必须先给"读题反馈"（题型判断、每问目标、数据观察、初步思路、风险点、待确认项），不能等到分析报告才输出想法。
3. 编程手得到真实结果后，先给"结果即时解读"（关键数值、合理性、异常原因、是否补检验）。
4. 论文交付或用户反馈"看完了"后，主动打开论文文件复查：按自查表输出缺陷清单与修改建议，提醒用户自查，并给出查重/AI 率检测的正确做法（官方无统一合格线，检测结果只作风险信号）。
5. 用户问"现在该做什么/进度如何"时，只汇报进度与建议，不重新跑全流程。

细节见 `references/进度状态机与主动建议.md`。
...
| 备赛咨询、规则/提交/AI 合规查询 | `references/2026国赛规则与AI合规.md` | 否 |
| 题型判定、模型选型咨询 | `references/题型识别与模型选型.md` | 否 |
| 论文写作规范、检验、降 AI 咨询 | `references/论文写作与检验规范.md` | 否 |
| 沿用用户偏好、复盘上次做题问题 | `references/实战复盘与用户偏好.md` | 否 |
| 问进度、下一步建议、论文复查 | `references/进度状态机与主动建议.md` | 否 |
...
2. 逐小问建立依赖链和承接关系：明确每问解决什么、向后续问题提供什么输入或约束，以及共享变量、目标函数和评价口径是否一致。
3. 不编造题目没有给出的背景、变量含义、约束、评价标准、数据来源、政策场景或结论要求；必须补充的内容只能作为有依据的模型假设，并说明影响。
4. 每道子问题最多使用两个独立模型体系。物理题中，同一控制方程或同一物理机制在不同近似阶次、边界条件或精度下的展开，计为一个模型族，不机械拆成多个模型；只有控制机理、状态变量或求解目标实质不同才另计模型。辅助的灵敏度分析、误差检验和可视化不计作独立模型。
5. 避免直接套用过于常见的简单模型来冒充创新；创新必须来自问题结构、数据处理、约束设计、算法改进或验证方式，并说明依据。
6. 数据判定标准按题目、官方规则、领域文献或数据分析结果确定。
7. 不因两个模型得到相近结论就强制删除其中一个；是否保留由研究目的和证据价值决定。
...
| 判定题型或模型选型 | `references/题型识别与模型选型.md` |
| 目标竞赛为国赛、处理提交材料或 AI 合规 | `references/2026国赛规则与AI合规.md` |
| 写论文、做三类检验或降 AI 痕迹 | `references/论文写作与检验规范.md` |
| 用户要求沿用个人偏好、复盘既往返工问题 | `references/实战复盘与用户偏好.md` |
| 汇报进度或给出下一步建议 | `references/进度状态机与主动建议.md` |
