...
同时明确篇幅质量目标。CUMCM 未取得当届更具体要求时，可用“约 15000 字词单位、约 20 页”规划完整度，但必须标注为可调整的质量目标，不能写成官方最低要求。以 2026 年官方规范为例，正文从纸质版第 4 页开始计算且不超过 30 页，摘要页和附录不计入正文页数，并未规定最低 15000 字或最低 20 页。

2026 年 CUMCM 还要求无论是否使用 AI，均在参考文献前设置“AI 工具使用声明”。使用 AI 时，支撑材料必须包含《AI 工具使用详情.pdf》；正文不需要逐处标注 AI 生成或辅助生成内容，参考文献也不列 AI 工具。
...
2. 建立小问叙事链和内部 Claim-Evidence 映射：为每个子问题列出核心主张、公式、结果表位置、至少 1 幅拟入文正式图、代码输出或已核验文献，并说明该问与前后问题的承接关系；不得只集中使用问题一的图。缺证据时回退到建模手或编程手，禁止编造。
3. 在开始长篇正文和双格式排版前，派发独立质检 Subagent 执行 `W1` 证据大纲门禁；未返回 `PASS` 不得先写后补。
4. 按官方结构写完整正文，保证问题分析、模型建立、结果分析和模型评价中的术语、符号、单位、结论口径一致；题目没有给出的说明不能补写成事实，必要补充只能列入模型假设并解释影响。
5. 引用由双引擎搜索结果和原始出版页面核验；章节之间用具体承接句说明“上一问产出如何进入下一问”，让评委通读时能顺读完整逻辑链。
6. 默认先确定同一份正文、数据、图表和参考文献，再生成 Word；用户显式要求 LaTeX 时同时生成 LaTeX，禁止两份论文出现不同结论。
7. Word 使用 `../../../tools/docx/SKILL.md` 构建 DOCX，公式使用原生 OMML；已有完整 LaTeX 主稿时可通过 `convert_latex` 生成内容一致的 Word 初稿，再按官方 DOCX 模板修正。LaTeX（可选）使用 `../../../tools/latex/SKILL.md` 复制完整官方模板项目、填充源码并真实编译 PDF。
8. 检查 Word 的结构、篇幅、公式、图表、全部子问题覆盖、编号引用、参考文献和实际渲染页数。用户显式要求 LaTeX 时，还必须消除编译错误及未解析的引用，核对权威资源—源码—PDF 哈希、PDF 总页数、正文页数、附录边界、字体嵌入、空白页、页面尺寸和图片 DPI；所有预警必须修正，或根据当届官方规则记录明确覆盖理由后才能继续。
9. 完成下列确定性门禁后，派发独立质检 Subagent 执行 `W2` 论文终检；未返回 `PASS` 不得宣称完成或交付论文。

## 阶段内独立门禁

- `W1`：质检 Subagent 核对每个必须回答的结论类型都有精确证据路径，摘要拟用关键数值与结果表一致，图表、公式和引用都有章节落点，并检查小问之间的依赖链、承接句和未给信息是否被误写成事实。用户显式要求 LaTeX 时，Word 与 LaTeX 共用同一证据源。证据大纲只保留在内部检查中，不新增固定交付物。
...
论文交付后，或在用户表示"看完了/改好了/帮我再看看"时，主动打开 `PROJECT_ROOT` 中的论文文件复查，按 `../../../references/2026国赛规则与AI合规.md` 和 `../../../references/论文写作与检验规范.md` 输出：

1. 复查结果：结构完整性、摘要（一页、量化数字、"针对问题 X"分段）、图表编号与正文引用、三类检验、公式、AI 工具使用声明位置、参考文献双向对应、匿名性、页数与体积。
2. 缺陷清单：按严重度排序并精确到章节/页码；能直接改的立即改，改后重新过对应门禁。
3. 自查引导：提醒用户按当届自查表维度自查，列出最该先看的 3–5 项（AI 声明真实性与位置、《AI 工具使用详情.pdf》完整性、数据与图表一致性、漏题、匿名、页数）。
4. 查重与 AI 率建议：官方无统一 AIGC 合格线，检测结果只作风险信号；改进方向是真实数据锚定、规范引用、AI 声明与实际一致；AI 给的文献/数字必须复核。

复查结论在最终回复中主动呈现，用户要求时才落盘为 `论文复查报告.md`（辅助文档，非固定交付物）。
...
| 用户要求沿用/学习个人参考论文版式 | `references/用户版式偏好.md` |
| 用户要求融合复盘要点或避免既往论文返工问题 | `../../../references/实战复盘与用户偏好.md` |
