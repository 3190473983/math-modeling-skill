...
用户要求学习或沿用个人参考论文版式时，在满足官方规则和官方模板后，再读取 `references/用户版式偏好.md` 套用视觉密度、标题层级、图表组织和结尾顺序。该偏好只学习格式，不复用参考论文内容。

### CUMCM 篇幅的硬约束与质量目标

- 2026 年官方《全国大学生数学建模竞赛论文格式规范》规定摘要原则上不超过一页、正文从纸质版第 4 页开始且不超过 30 页，摘要页和附录不计入正文页数，没有规定最低 15000 字或最低 20 页。
- 为防止论文过短，工具默认用约 15000 字词单位和约 20 页作为 CUMCM 完整度质量目标；它们不是官方最低要求，可由当届规则或用户要求覆盖。
- 页数必须以 DOCX 渲染成 PDF 后的实际页数为准，不能根据字符数猜测；校验时必须分别记录摘要页、正文起始页、附录起始页和正文页数，不得用 PDF 总页数或"摘要到参考文献"页数替代正文页数。

### CUMCM 2026 AI 声明

- 无论是否使用 AI，均须在参考文献前设置"AI 工具使用声明"。
- 使用 AI 工具生成或辅助生成的内容，不再要求在正文相应位置逐处标注，也不在参考文献中列出所用 AI 工具。
- 使用 AI 时，支撑材料必须包含《AI 工具使用详情.pdf》，说明工具名称、版本或型号、具体使用目的和环节、主要提示方式和使用过程、采纳内容、人工修改与核验情况。
...
该检查会报告篇幅、公式/图/表数量、图表编号与正文引用、参考文献双向对应、AI 声明位置以及页数问题。默认 15000 字词单位、20 页、公式 5 个、图 8 幅、表 3 个均为 CUMCM 完整度质量目标；其他竞赛同样默认至少 8 幅正式图。2026 年 CUMCM 正文 30 页上限和 AI 声明置于参考文献前属于已经核验的官方硬约束。使用其他届次或竞赛时必须传入相应阈值；降低默认目标必须记录官方条款或用户要求。OOXML 校验通过后仍需渲染抽检，因为 XML 合法不等于版面正确。
